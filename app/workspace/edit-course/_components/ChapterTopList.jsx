...
function ChapterTopList({ course }) {
  ...
  return (
    <div>
        <h2 className='text-lg font-bold mt-4'>Chapters with their Topics</h2>
        ...
        {chapter?.topics.map((topic, index) => (
          <div className='flex flex-col items-center' key={index}>
            ...
            {index == chapter?.topics?.length - 1 && <div className='flex items-center gap-5'>
              <ThumbsUp className='text-center rounded-full bg-gray-300 h-14 w-14 text-gray-500 p-4' />
            </div>}
            {index == chapter?.topics?.length - 1 && <div className='h-10 bg-gray-300 w-1'></div>}
          </div>
        ))}
  ...
  <div className='p-4 border shadow rounded-xl bg-gray-600 text-white'>
    <h2>Finish</h2>
  </div>
</div>
